...
test("server-renders the backend landing page", async () => {
  ...
  assert.match(html, /Start[\s\S]*Tools[\s\S]*Structured Outputs[\s\S]*Files[\s\S]*Components[\s\S]*Storage[\s\S]*Subagents/);
  ...
  assert.match(text, /component: openai\(\)[\s\S]*agentloop: pi\(\)/);
  ...
  assert.match(text, /lookupStock[\s\S]*app\([\s\S]*store-api[\s\S]*component: openai\(\)[\s\S]*agentloop: pi\(\)/);
  assert.match(html, /Explicit agent-loop policy such as Pi or Codex/);
  assert.match(html, /Managed tools carry one immutable runtime bundle/);
...
test("server-renders the concise product documentation", async () => {
  ...
  assert.match(text, /component: openai\(\)[\s\S]*agentloop: pi\(\)[\s\S]*tools: \[bash\(\), read\(\), write\(\)\]/);
  assert.match(text, /\.client/);
  assert.match(text, /copyFromSandbox/);
  assert.match(text, /exactly one[\s\S]*app\(\) Environment/);
