...
test("server-renders the agent backend landing page", async () => {
  ...
  assert.match(html, /<title>The agent backend for AI workloads<\/title>/i);
  assert.match(html, /rel="icon"[^>]+href="\/icon\.svg/i);
  assert.match(html, /The agent backend for AI workloads/);
  assert.match(html, /simple, intuitive, session-oriented SDK/i);
  assert.match(html, /Docs[\s\S]*Dashboard[\s\S]*GitHub/);
  assert.match(html, /role="tablist"/);
  assert.match(html, /Start[\s\S]*Custom tool[\s\S]*Structured output[\s\S]*Continue/);
  ...
  assert.match(html, /<h3>Brain<\/h3>[\s\S]*<h3>Hands<\/h3>/);
