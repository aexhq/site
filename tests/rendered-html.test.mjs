...
test("server-renders the backend landing page", async () => {
  ...
  assert.match(html, /Start[\s\S]*Tools[\s\S]*Structured Outputs[\s\S]*Files[\s\S]*Binding[\s\S]*Storage[\s\S]*Subagents/);
  assert.doesNotMatch(html, />Continue<\/button>/);
  assert.match(html, /<pre class="site-code"/);
  assert.match(text, /tool/);
  assert.match(text, /inspectFile/);
  assert.match(text, /\.setup/);
  assert.match(text, /lookupOrder\.bind\(application\)/);
  ...
  assert.match(text, /runtime\.files\.upload/);
  assert.match(text, /bash\(\)\.bind\(workspace\)/);
  assert.match(text, /session\.storage\.copyFromEnvironment/);
  ...
  assert.match(html, /<h3>Database<\/h3>[\s\S]*<h3>Storage<\/h3>[\s\S]*<h3>Your app<\/h3>[\s\S]*<h3>Brain<\/h3>[\s\S]*<h3>Extensions<\/h3>[\s\S]*<h3>Environment<\/h3>/);
  assert.doesNotMatch(html, /anthropic\.com\/engineering\/managed-agents/);
  assert.match(html, /<h2 id="features-title">Features<\/h2>/);
  assert.match(html, /brain-extension-features-title[\s\S]*tool-extension-features-title[\s\S]*environment-extension-features-title/);
  assert.match(text, /lookupStock[\s\S]*app\([\s\S]*store-api[\s\S]*loop: pi\(\)/);
  assert.match(html, /Explicit agent-loop policy such as Pi or Codex/);
  assert.match(html, /Prepared runtimes and one-time setup travel with the tool/);
...
test("server-renders the concise product documentation", async () => {
  ...
  assert.match(text, /lookupStock[\s\S]*app\([\s\S]*store-api[\s\S]*loop: pi\(\)/);
  assert.match(text, /\.setup/);
  assert.match(text, /copyFromEnvironment/);
  assert.match(text, /tool\.bind\(environmentRef\)/);
